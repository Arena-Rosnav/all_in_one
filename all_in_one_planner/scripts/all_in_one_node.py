...
from visualizer import AllInOneVisualizer
...
class AioNode:
    def __init__(self):
        ...
        paths = {
            "model": os.path.join(base_dir, "AIO_agents", agent_aio),
            "drl_agents": os.path.join(
                base_dir, "scripts", "AIO_local_planners", "rosnav_drl"
            ),
            "all_in_one_parameters": os.path.join(
                base_dir, "AIO_agents", agent_aio, "all_in_one_parameters.json"
            ),
        }

        if not os.path.exists(paths["model"]):
            rospy.logerr(
                "No AIO planner defined for robot model " + self._robot_model + "! Exit"
            )
            sys.exit(1)
        ...
        self.aio_timer = rospy.Timer(
            rospy.Duration(0, self._base_control_interval), self.aio_control_sequence
        )
    ...
    def load_aio_policy(self, model_path):
        ...
        assert os.path.isfile(model_parameters_path), (
            "No model file found in %s" % model_parameters_path
        )
        assert os.path.isfile(vecnorm_path), (
            "No vecnorm file found in %s" % vecnorm_path
        )
        ...
        # load agent
        self._aio_policy = PPO.load(
            model_parameters_path, custom_objects=custom_objects
        ).policy
    ...
    def aio_control_sequence(self, _):
        ...
        if not goal_reached:
            ...
            self.cmd_vel = self.local_planner_manager.execute_local_planner(
                self.aio_action, obs_dict
            )
        ...
        if time_interval_ms > 100:
            rospy.logwarn(
                "AIO Planner: Time interval of 100ms exceeded! "
                + str(time_interval_ms)
                + "ms needed!"
            )
    ...
    def _extract_step_parameters(self, config_path: str):
        ...
        if "update_global_plan_frequency" in config_data:
            self._update_global_plan_frequency = config_data[
                "update_global_plan_frequency"
            ]
        else:
            rospy.logwarn(
        ...
        if "all_in_one_planner_frequency" in config_data:
            self._all_in_one_planner_frequency = config_data[
                "all_in_one_planner_frequency"
            ]
        else:
            rospy.logwarn(
