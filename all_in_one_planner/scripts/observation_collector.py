...
class ObservationCollectorAllInOne:
    def __init__(self, required_obs: dict, all_in_one_config: str):
        ...
        self.max_range_aio_planner = rospy.get_param("laser/range")
        self.robot_radius = rospy.get_param("robot_radius")

        (
            self._reduced_num_laser_beams,
            self._laser_stack_size,
            self._add_robot_velocity,
            self._use_dynamic_scan,
        ) = self._extract_obs_space_info(all_in_one_config)
        self._full_laser_range = self._reduced_num_laser_beams < 0
        self._laser_vec_size = (
            self._laser_num_beams
            if self._full_laser_range
            else self._reduced_num_laser_beams
        )
        ...
        rospy.loginfo(
            "Use laser info of size ({0},{1})".format(
                self._laser_stack_size, self._laser_vec_size
            )
        )
        ...
        self._dynamic_scan_pub = rospy.Publisher(
            "all_in_one_planner/scan_dynamic", LaserScan, queue_size=1
        )
        self._subgoal_visualizer = rospy.Publisher(
            "all_in_one_planner/vis_subgoal",
    ...
    def get_observations(self, make_new_global_plan: bool = False):
        ...
        if make_new_global_plan or self._global_plan_service is None:
            ...
            if service_available:
                ...
                self._subgoal = self._extract_subgoal(
                    self._globalplan, self._drl_subgoal_horizon
                )
                self._subgoal_reward = self._extract_subgoal(
                    self._globalplan, self._AIO_subgoal_horizon
                )
                # visualize subgoal
        ...
        (
            rho_local,
            theta_local,
        ) = ObservationCollectorAllInOne._get_goal_pose_in_robot_frame(
        ...
        (
            local_goal_x,
            local_goal_y,
        ) = ObservationCollectorAllInOne._get_local_goal_in_robot_frame_xy(
            self._subgoal, self._robot_pose
        )

        (
            subgoal_reward_robot_frame_rho,
            subgoal_reward_robot_frame_theta,
        ) = ObservationCollectorAllInOne._get_goal_pose_in_robot_frame(
        ...
        merged_obs = np.float32(
            np.hstack(
                [
                    laser_vec,
                    np.array([rho_global, theta_global]),
                    np.array([rho_local, theta_local]),
                ]
            )
        )
        ...
        if self._add_robot_velocity:
            merged_obs = np.hstack(
                [merged_obs, [self._twist.linear.x, self._twist.angular.z]]
            )

        obs_dict = {
            "laser_scan": scan_full,
            "goal_map_frame": self._subgoal,
            "goal_reward_robot_frame": [
                subgoal_reward_robot_frame_rho,
                subgoal_reward_robot_frame_theta,
            ],
    ...
    def reset(self):
        ...
        if self._needs_last_three_laser:
            ...
            self._last_three_laser_scans = np.array([scan, scan, scan]).transpose()
    ...
    def get_dynamic_scan(self, static_scan: np.ndarray, scan: np.ndarray):
        ...
        dynamic_scan = np.where(
            dynamic_scan > 3, self.max_range_aio_planner, dynamic_scan
        )
        dynamic_scan = np.where(
            dynamic_scan < self.robot_radius, self.max_range_aio_planner, dynamic_scan
        )

        # filter outliers
        outlier_min_are = int(0.025 * self._laser_num_beams)
        dynamic_scan = self.filter_outliers(
            dynamic_scan, self.max_range_aio_planner, outlier_min_are
        )
    ...
    def get_sync_obs(self):
        ...
        if self._use_dynamic_scan:
            ...
            if laser_stamp is not None:
                while (
                    not abs(laser_stamp > static_laser_stamp) <= self._sync_slop
                    and len(self._laser_static_deque) > 0
                ):
        ...
        # print(f"Laser: {laser_scan}")
        # print(f"static: {static_scan}")
    ...
    def callback_global_plan(self, msg_global_plan: nav_msgs.msg.Path):
        self._new_global_plan = True
        self._globalplan = ObservationCollectorAllInOne.process_global_plan_msg(
            msg_global_plan
        )
    ...
    def _start_global_planner(self):
        ...
        config_path = os.path.join(
            rospkg.RosPack().get_path("all_in_one_planner"),
    ...
    def _wait_for_global_plan_service(self) -> bool:
        ...
        for i in range(max_tries):
            if (
                make_plan_service_name in service_list
                and reset_costmap_service_name in service_list
            ):
                break
            else:
                time.sleep(0.3)
        ...
        if (
            ...
            self._global_plan_service = rospy.ServiceProxy(
                make_plan_service_name,
                all_in_one_global_planner_interface.srv.MakeNewPlan,
                persistent=True,
            )
            self._reset_global_costmap_service = rospy.ServiceProxy(
                reset_costmap_service_name, std_srvs.srv.Empty, persistent=True
            )
            return True
        else:
            return False
    ...
    def _calc_reduced_laser_vec(self, laser_scan: np.array):
        return list(
            map(np.min, np.array_split(laser_scan, self._reduced_num_laser_beams))
        )
    ...
    def _extract_obs_space_info(all_in_one_config_path: str):
        ...
        assert (
            all_in_one_config is not None
        ), "All in one parameter file cannot be found!"

        if "observation_space" not in all_in_one_config:
            rospy.logwarn(
                "No specification of the observation space found. Use default values!"
            )
            return -1, 1, False, True
        else:
            obs_info = all_in_one_config["observation_space"]
            assert "laser_range" in obs_info, (
                "Please specify the laser_range parameter in the observation "
                "space section of the aio config file!"
            )
            if obs_info["laser_range"] == "full":
                reduced_laser_size = -1
            else:
                assert "size_laser_vector" in obs_info, (
                    "Please specify the size_laser_vector parameter in the "
                    "aoi config file or use laser_range=full setting otherwise!"
                )
                reduced_laser_size = obs_info["size_laser_vector"]
            assert "laser_stack_size" in obs_info, (
                "Please specify the laser_stack_size parameter in the aoi "
                "config file!"
            )
            ...
            if "use_dynamic_scan" not in obs_info or (
                "use_dynamic_scan" in obs_info and obs_info["use_dynamic_scan"]
            ):
    ...
    def process_global_plan_msg(globalplan):
        global_plan_2d = list(
            map(
                lambda p: ObservationCollectorAllInOne.pose3D_to_pose2D(p.pose),
                globalplan.poses,
            )
        )
    ...
    def filter_outliers(scan: np.ndarray, max_range: float, outlier_range: int):
        ...
        for i in range(1, scan.size):
            if abs(scan[i - 1] - scan[i]) > 0.1:
                if (
                    area_size_counter <= outlier_range
                    and abs(scan[i] - max_range) > 0.1
                ):
