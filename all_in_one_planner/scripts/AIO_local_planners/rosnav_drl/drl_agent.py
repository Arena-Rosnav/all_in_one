...
class DrlAgent(ModelBase):
    def __init__(self, save_path: str, name: str):
        observation_information = {"laser_scan": True, "goal_in_robot_frame": True}
        super(DrlAgent, self).__init__(observation_information, "rosnav-drl")

        robot_model = rospy.get_param("robot_model")
        base_dir = rospkg.RosPack().get_path("rosnav")
        agent_dir = os.path.join(base_dir, "agents", robot_model)

        model_save_path = os.path.join(agent_dir, "best_model.zip")
    ...
    def get_next_action(self, observation_dict, observation_array=None) -> np.ndarray:
        ...
        if observation_array is None:
            ...
            merged_obs = np.hstack(
                [
                    observation_dict["laser_scan"],
                    np.array(observation_dict["goal_in_robot_frame"]),
                    np.array(
                        [obs_twist.linear.x, obs_twist.linear.y, obs_twist.angular.z]
                    ),
                ]
            )
        else:
